...
class LcardVACPlot_Interface(object):
        def __init__(self, Lcard_device):
            ...
            self.QTimer_updatePlot = QtCore.QTimer()
            self.QTimer_updatePlot.timeout.connect(self._updatePlot)
            self.Y_x_plot = None
        ...
        def _updatePlot(self):
            ...
            if self.Y_x_plot is None:
                return
            LastData = self.myLcard_IFFB.getNumpyData()
            try:
                ...
                print(">>4")
                x_data = str_to_channel_data(LastData, x_label)[start:end]
                y_data = str_to_channel_data(LastData, y_label)[start:end]
                print(">>5")
        ...
        def setIsPlotTimerActive(self, active_state : bool):
            if self.IsPlotUpdateActive == active_state:
                return
            self.IsPlotUpdateActive = active_state
            if active_state:
                self.QTimer_updatePlot.start()
            else:
                self.QTimer_updatePlot.stop()
