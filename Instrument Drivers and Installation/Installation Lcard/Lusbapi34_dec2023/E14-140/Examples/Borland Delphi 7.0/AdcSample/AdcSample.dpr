...
begin
	...
	pModule := CreateLInstance(pAnsiChar('e140'));
	...
	while true do
		begin
			// выведем очередной отсчёт на первый канал АЦП: дифф.подключение, 2.5 В входной диапазон
			if not pModule.ADC_SAMPLE(@AdcSample0, 0 or (0 shl 4) or (InputRangeIndex shl 6)) then begin WriteLn(#10#13' ADC_SAMPLE(0, ) -> Bad!'); break; end
				// выведем очередной отсчёт на второй канал АЦП: измерение собственного напряжения нуля, 2.5 В входной диапазон,
