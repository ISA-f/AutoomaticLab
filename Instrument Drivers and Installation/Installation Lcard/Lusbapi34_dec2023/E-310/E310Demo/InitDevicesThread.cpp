...
void TInitDevicesThread::EnableModuleControlElements(bool EnabledFlag)
{
	...
	MainForm->ModuleDetectionShape->Brush->Color = EnabledFlag ? (IsModuleLoaded ? clLime : clRed) : clBtnShadow;
	// надпись около лампочки обнаружения модуля
	MainForm->ModuleDetectionLMDLabel->Enabled = EnabledFlag ? true : false;
	// statictext c номером виртуального слота
	MainForm->VirtualSlotLMDLabel->Enabled = EnabledFlag ? true : false;
	Str = IsModuleLoaded ? (AnsiString)(VirtualSlot) : (AnsiString)"";
	MainForm->VirtualSlotLMDLabel->Caption = EnabledFlag ? Str.c_str() : "";
	// надпись номера виртуального слота
	MainForm->VirtualSlotLabelLMDLabel->Enabled = EnabledFlag ? true : false;
}
...
/*	if(MainForm->GeneratorPars.DdsPars.ControlReg.BitFields.SYNCOUTEN	&&
	...
	// если нужно - сбросим признак запуска приложения
	if(IsModuleLoaded)	MainForm->IsProgramLaunching = false;
	else						MainForm->IsProgramLaunching = true;
}
