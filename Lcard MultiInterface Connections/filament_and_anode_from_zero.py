...
class new_Filament_and_Anode:
        def __init__(self, my_device = 'Device_LcardE440', experiment_ini = "Constants.ini"):
                self.experiment_ini = experiment_ini
        ...
        def retranslateUi(self, MainWindow):
                ...
                self.pushButton_start.clicked.connect(self.StartMeasurements)
        
        def ConnectToPhysicalDevices(self):
        ...
        def StartMeasurements(self, filename: str):
            self.myLcard.StartMeasurements("LcardE2010B_" + filename)

        def FinishMeasurements(self):
            self.myLcard.FinishMeasurements()

        def StartUI(self):
            self.myThread = threading.Thread(target = self.UpdateUIinThread,
                daemon = False) # обновлением ui будет заниматься отдельный поток
            self.UIIsActive = True
            self.myThread.start()

        def StopUI(self):
            self.UIIsActive = False #говорим дочернему потоку, что пора заканчивать
            self.myThread.join() #ожидаем завершения дочернего потока
            self.myThread = None
            
        def UpdateUIinThread(self):
            while(self.UIIsActive):
                self.UpdateUIonce()
                time.sleep(0.5)

        def UpdateUIonce(self):
                print("Updating UI")
                """
                # for filament
                ...
                """
                # for anode
                Data = self.myLcard.ReadFlashDataAveraged()
                print(Data)
                lcard_time = Data[0]
                ch1_mean = Data[1]
                ch1_var = Data[2]
                ...
                config.read(self.experiment_ini)
                print(self.my_confog_lcard)
...
if __name__ == "__main__":
    ...
    FA = new_Filament_and_Anode(my_device = 'Device_LcardE2010B')
    #FA.StartUI()
    #FA.SetupUI(MainWindow)
    #MainWindow.show()
    FA.StartMeasurements("new_FA.txt")
    time.sleep(1)
    FA.FinishMeasurements()
    #app.exec_()
